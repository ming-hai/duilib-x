...
void CxImage::SetPalette(uint32_t n, uint8_t *r, uint8_t *g, uint8_t *b)
{
    ...
    if (!b) b = g;
    RGBQUAD* ppal=GetPalette();
    uint32_t m=std::min(n, (uint32_t)head.biClrUsed);
...
void CxImage::SetPalette(rgb_color *rgb,uint32_t nColors)
{
    ...
    RGBQUAD* ppal=GetPalette();
    uint32_t m=std::min(nColors, (uint32_t)head.biClrUsed);
    for (uint32_t i=0; i<m;i++){
...
void CxImage::SetPalette(RGBQUAD* pPal,uint32_t nColors)
{
    ...
    memcpy(GetPalette(),pPal,std::min(GetPaletteSize(), (uint32_t)(nColors*sizeof(RGBQUAD))));
    info.last_c_isvalid = false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
void CxImage::SetClrImportant(uint32_t ncolors)
{
    ...
    case 1:
        head.biClrImportant = std::min(ncolors, (uint32_t)2);
        break;
    case 4:
        head.biClrImportant = std::min(ncolors, (uint32_t)16);
        break;
    case 8:
...
bool CxImage::SetRectColor(int32_t left, int32_t top, int32_t right, int32_t bottom, RGBQUAD color, bool bSetAlpha)
{
    ...
    int32_t startx = std::max(0, std::min(left, (int32_t)head.biWidth));
    int32_t endx = std::max(0, std::min(right, (int32_t)head.biWidth));
    int32_t starty = head.biHeight - std::max(0, std::min(top, (int32_t)head.biHeight));
    int32_t endy = head.biHeight - std::max(0,std::min(bottom, (int32_t)head.biHeight));
