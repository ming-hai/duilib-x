    ...
    for (int32_t y=0;y<head.biHeight;y++){
        ...
        for (int32_t x=0;x<head.biWidth;x++){
            if (!errordiffusion){
                ...
                c.rgbRed = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbRed + ((er*7)/16)));
                c.rgbGreen = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbGreen + ((eg*7)/16)));
                c.rgbBlue = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbBlue + ((eb*7)/16)));
                ...
                for(int32_t i=-1; i<2; i++){
                    ...
                    c.rgbRed = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbRed + ((er * coeff)/16)));
                    c.rgbGreen = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbGreen + ((eg * coeff)/16)));
                    c.rgbBlue = (uint8_t)std::min(255L,std::max(0L,(int32_t)c.rgbBlue + ((eb * coeff)/16)));
...
bool CxImage::Crop(int32_t left, int32_t top, int32_t right, int32_t bottom, CxImage* iDst)
{
    ...
    int32_t startx = std::max(0L,std::min(left,head.biWidth));
    int32_t endx = std::max(0L,std::min(right,head.biWidth));
    int32_t starty = head.biHeight - std::max(0L,std::min(top,head.biHeight));
    int32_t endy = head.biHeight - std::max(0L,std::min(bottom,head.biHeight));
...
//                    nx=std::max(xmin,std::min(nx,xmax));
//                    ny=std::max(ymin,std::min(ny,ymax));
                }
                else { nx=-1;ny=-1;}
                if (head.biClrUsed==0){
